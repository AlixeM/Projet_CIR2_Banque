...
Customer get_a_customer_from_a_ptree(ptree &pt)
{
    ...
    std::string alors = pt.get<std::string>("Alors");
    std::vector<int> account_numbers;
    std::string test = pt.get<std::string>("Test");
  ...
  Customer customer(number, std::move(name),std::move(alors), std::move(account_numbers),std::move(test));
  return customer;
}
*/